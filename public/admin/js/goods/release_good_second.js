...
//		$('#presell_day').parents('dl').addClass('hide');
	 ...
	 * 循环处理价格 不让价格为空
	 */
	$('input[name="market_price"],input[name="sku_price"],input[name="mprice"],input[name="cost_price"],input[name="stock_num"],input[name="code"]').live('keyup',function() {
		var $this = $(this);
		var reg = /^\d+(.{0,1})\d{0,2}$/;
		if($this.attr("name") == "sku_price" || $this.attr("name") == "market_price"|| $this.attr("name") == "mprice" || $this.attr("name") == "cost_price" || $this.attr("name") == "stock_num"){
	 ...
	 * 离开焦点事件也要进行处理
	 */
	$('input[name="market_price"],input[name="sku_price"],input[name="mprice"],input[name="cost_price"],input[name="stock_num"],input[name="code"]').live("blur",function(){
		$(this).keyup();
	});
...
//	 			}
			...
			if(is_update){
				...
				if($(".edit-sku-batch-info .js-sku-price").val() != ""){
     			...
     			if($(".edit-sku-batch-info .js-mprice").val() != ""){
					$(".block-goods-sku>.goods-sku-list .input-common[name='mprice']").val($(".edit-sku-batch-info .js-mprice").val());
					$("#txtProductMprice").val($(".edit-sku-batch-info .js-mprice").val());
				}
		...
		if(v.length>0){
			...
			if(is_update){
				...
				if(tag == "sku-price"){
					...
					$(".block-goods-sku>.goods-sku-list .input-common[name='mprice']").val(v);
...
function ValidateUserInput() {
	...
	//VIP会员价格
	// if (!IsNum("#txtProductMprice") || parseFloat($("#txtProductMprice").val()) < 0) {
	// 	$(".goods-nav ul li:eq(0)").click();
	// 	$("#txtProductMprice").nextAll("span:last").text("VIP会员价格不能为空，且不能为负数").show();
	// 	$("#txtProductMprice").focus();
	// 	return false;
	// } else {
	// 	$("#txtProductMprice").nextAll("span:last").hide();
	// }
	//保质期天数
...
function PackageProductInfo() {
	...
	productViewObj.mprice = $("#txtProductMprice").val();//.replace(/^\s*/g, "").replace(/\s*$/g, "") == "" ? 0 : $("#txtProductMprice").val().replace(/^\s*/g, "").replace(/\s*$/g,"");// VIP会员价
...
/**
 * 循环VIP会员价 2016年12月2日 11:55:30
 */
function eachMprice() {
	var $price = 0;
	$.each($('input[name="mprice"]'), function(i, item) {
		var $this = $(item);
		console.log($this);
		var num = $this.val();
		var numint = parseFloat(num);
