...
function createTable(){
	...
	if($specObj.length == 0){
		...
		$("#txtProductMprice").val(0).removeAttr("readonly");
		...
		if($("#txtProductMprice").attr("readonly") != "readonly"){
			$("#txtProductMprice").val(0).attr("readonly","readonly");
		}
	...
	for(var i = 0; i < $sku_array.length; i ++){
		...
		if($temp_Obj[child_id_string] == undefined){
			...
			$temp_Obj[child_id_string]["mprice"] ="0.00";
		...
		html += '<tr skuid="' + child_id_string + '">';
			...
			//VIP会员价格
			html += '<td>';
				html += '<input type="text" class="input-common middle js-mprice" maxlength="10" name="mprice" value="'+$temp_Obj[child_id_string]["mprice"]+'"/>';
				html += '<span class="help-inline" ' + help_inline_style + '>VIP会员价最小为 0.01</span>';
			html += '</td>';
...
function synchroSkuValueData(){
	...
	for(var i = 0; i < $sku_array.length ; i++ ){
		...
		if(sku_str == ""){
			sku_str = sku_id +"¦"+value_array["market_price"]+"¦"+value_array["sku_price"]+"¦"+value_array["mprice"]+"¦"+value_array["cost_price"]+"¦"+value_array["stock_num"]+"¦"+value_array["code"];
		}else{
			sku_str +="§"+sku_id +"¦"+value_array["market_price"]+"¦"+value_array["sku_price"]+"¦"+value_array["mprice"]+"¦"+value_array["cost_price"]+"¦"+value_array["stock_num"]+"¦"+value_array["code"];
		}
	}
	...
	if($specObj.length > 0)
	{
		if(sku_list!=null){
			$.each(sku_list,function(c,v){
				...
				$temp_Obj[v.attr_value_items]["mprice"] =v.mprice;
