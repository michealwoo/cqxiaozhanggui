...
class Goods extends BaseService implements IGoods
{
     ...
     */
    public function getGoodsRankViewList($page_index = 1, $page_size = 0, $condition = '', $order = 'ng.sort asc',$uid)
    {
        $goods_model = new NsGoodsModel();
        // 针对商品分类
        $viewObj = $goods_model->alias("ng")
            ->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
            ->field("ng.goods_id,ng.goods_name,ng_sap.pic_cover_mid,ng.promotion_price,ng.mprice,ng.market_price,ng.goods_type,ng.stock,ng_sap.pic_id,ng.max_buy,ng.state,ng.is_hot,ng.is_recommend,ng.is_new,ng.sales,ng_sap.pic_cover_small");
    ...
    private function addOrUpdateGoodsSkuItem($goods_id, $sku_item_array)
    {
        ...
        if (empty($sku_count)) {
            $data = array(
                ...
                'market_price' => $sku_item[1],
                'promote_price' => $sku_item[1],
                'price' => $sku_item[2],
                'mprice' => $sku_item[3],
                'cost_price' => $sku_item[4],
                'stock' => $sku_item[5],
                'code' => $sku_item[6],
                'picture' => 0,
                'QRcode' => '',
