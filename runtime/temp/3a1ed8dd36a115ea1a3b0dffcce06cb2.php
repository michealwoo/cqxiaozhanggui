...
<?php endif; ?>
	...
	<?php if(count($goods_spec_array) > 0): if(is_array($goods_spec_array) || $goods_spec_array instanceof \think\Collection || $goods_spec_array instanceof \think\Paginator): if( count($goods_spec_array)==0 ) : echo "" ;else: foreach($goods_spec_array as $key=>$vo): ?>
			...
			<dl class="condition-item" <?php if($curr > 4): ?>style="display:none";<?php endif; ?>>
				...
				<!-- 更多 -->
				<div class="moreAndmulti">
					<?php if(count($goods_spec_array) > 10): ?>
	...
	<!-- 属性筛选 start -->
	<?php if(count($attr_or_spec) > 0): if(is_array($attr_or_spec) || $attr_or_spec instanceof \think\Collection || $attr_or_spec instanceof \think\Paginator): if( count($attr_or_spec)==0 ) : echo "" ;else: foreach($attr_or_spec as $key=>$vo): ?>
			<!-- <?php echo $curr += 1; ?> -->
			<?php if($vo['type'] != 1 and $vo['value'] != ''): if($vo['is_search'] == '1'): ?>
					<dl class="condition-item" <?php if($curr > 4): ?>style="display:none";<?php endif; ?>>
						...
						<!-- 更多 -->
						<div class="moreAndmulti">
							<?php if(count($vo['value_items']) > 10): ?>
							<span class="select-more"  is-show="0" data-height="37" onclick="showMore(this);">
								<i class="fa fa-chevron-down"></i><?php echo lang('goods_more'); ?>
							</span>
							<?php endif; ?>
	...
	<?php if($max_price=='' && $category_count>0): ?>
		...
		<dl class="condition-item" <?php if($curr > 4): ?>style="display:none";<?php endif; ?>>
			...
			<!-- 更多 -->
			<div class="moreAndmulti">
				<?php if(count($category_price_grades) > 10): ?>
				<span class="select-more"  is-show="0" data-height="37" onclick="showMore(this);">
					<i class="fa fa-chevron-down"></i><?php echo lang('goods_more'); ?>
				</span>
				<?php endif; ?>
...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_716391482__=1;$__FOR_END_716391482__=$page_count+1;for($i=$__FOR_START_716391482__;$i < $__FOR_END_716391482__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_1665939839__=1;$__FOR_END_1665939839__=$page_num+1;for($i=$__FOR_START_1665939839__;$i < $__FOR_END_1665939839__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_172743834__=$page_count-$page_num+1;$__FOR_END_172743834__=$page_count+1;for($i=$__FOR_START_172743834__;$i < $__FOR_END_172743834__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_183675467__=$page-($page_num-1)/2;$__FOR_END_183675467__=$page+($page_num-1)/2+1;for($i=$__FOR_START_183675467__;$i < $__FOR_END_183675467__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
