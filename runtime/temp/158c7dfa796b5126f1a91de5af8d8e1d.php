<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:36:"template/shop/blue/Login/mobile.html";i:1545820112;s:42:"template/shop/blue/Login/registerBase.html";i:1545820112;s:32:"template/shop/blue/urlModel.html";i:1545820112;s:35:"template/shop/blue/loginTopNav.html";i:1545820112;}*/ ?>
<!DOCTYPE html>
	...
	$("#btn_submit").click(function(){
		...
		<?php if($login_verify_code['pc'] == 1): ?>
			if(vertification == ''){
				...
				$("#qrrepass").text("");
				$("#repass").css("border","1px solid #ccc");
				$("#vertify").css("color","red").text("<?php echo lang('please_enter_verification_code'); ?>");
				return false;
			}else{
				...
				success:function(data){
					if (data['code'] == 1) {
						...
						result = true;
							...
							$("#vertify").css("color","red").text("<?php echo lang('member_verify_code_error'); ?>");
		...
		//短信开启
	 	if(code.length != 6){
			...
			$("#qryzm").css("color","red").text("<?php echo lang('enter_correct_authentication_code'); ?>");
			$("#repass").css("border","1px solid #ccc");
		...
		$.ajax({
			...
			success:function(data){
				...
				if (data['code'] == 0) {
					if(agree == '0'){
						$("#is_agree").css("color","red").text("<?php echo lang('agreement_registration_agreement'); ?>");
						res = true;
					}else{
						$("#is_agree").css("color","red").text("");
					}
				}else {
					$("#qryzm").css("color","red").text("<?php echo lang('sms_verification_code_error'); ?>");
					res = true;
				}
			}
		}); 
...
<div class="header w990">
	<div class="logo-info">
		<a href="<?php echo __URL('SHOP_MAIN'); ?>" class="web-logo-a">
			<img alt="logo" src="<?php echo __IMG($web_info['logo']); ?>" class="weblogo"/>
		</a>
	</div>
</div>
						...
						<form id="MobileRegisterModel" class="form-horizontal" name="MobileRegisterModel" action="<?php echo __URL('SHOP_MAIN/login/mobile'); ?>" method="post">
							...
							<div class="form-group form-group-spe">
								...
								<div class="form-control-box">
									<input type="text" id="mobile" class="form-control" name="mobile" placeholder="<?php echo lang('input_your_mobile_phone_number'); ?>" />
							...
							<div class="form-group form-group-spe">
								...
								<div class="form-control-box">
									<input type="password" id="repass" class="form-control" name="repass" autocomplete="off" placeholder="<?php echo lang('confirm_the_password'); ?>">
							...
							<div class="form-group form-group-spe">
								...
								<div class="form-control-box" style="width: 450px;">
									<input type="text" id="captcha" name="captcha" tabindex="2" placeholder="<?php echo lang('please_enter_verification_code'); ?>" autocomplete="off" class="text text-te" style="width: 100px;"/>
							...
							<?php if($notice['noticeMobile'] == 1): ?>
			 				<div class="form-group form-group-spe">
								<label for="mobileregistermodel-sms_captcha" class="input-left">
									<span class="spark">*</span>
									<span><?php echo lang('sms_check_code'); ?>：</span>
								</label>
								<div class="form-control-box">
									...
									<input type="button" class="phonecode" id="sendOutCode" name="code_mobile" value="<?php echo lang('get_sms_check_code'); ?>">
									<span id="qryzm"></span>
								</div>
								<div class="invalid"></div>
							</div>
							...
							<div class="reg-btn">
								<div class="form-group form-group-spe">
									...
									<div class="form-control-box">
										<input type="button" class="btn-img btn-entry bg-color" id="btn_submit" name="btn_submit" value="<?php echo lang('agreement_and_registrationr'); ?>" title="<?php echo lang('fill_in_accordance_requirements'); ?>" />
									</div>
									<div class="invalid"></div>
								</div>
							</div>
						</form>
						<div id="message" style="color:red;width: 170px;height: 30px;margin-left: 160px;margin-top: -36px; display:none"><?php echo lang('must_meet_rules_register'); ?></div>
					</div>
				</div>
			</div>
		</div>
