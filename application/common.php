...
/**
 * 判断当前用户是否为VIP用户 返回相应的价格
 * @return string
 */
function ismprice($uid,$goods_id)
{
   
   $level = db('ns_member')->where(array('uid'=>$uid))->value('member_level');

   $info = db('ns_goods')->field('mprice,price')->where(array('goods_id'=>$goods_id))->find();

   if($level==48)   //member_level=48为VIP会员
   {
       $price = floatval($info['mprice']);
   }
   else
   {
...
//获取VIP价格
function getmprice($goods_id)
{
   
   $info = db('ns_goods')->field('mprice,price')->where(array('goods_id'=>$goods_id))->find();

   return floatval($info['mprice']);
